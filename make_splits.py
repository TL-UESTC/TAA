...
def main(dataset_name,
    ...
    droplistlen = (np.random.random_sample((all_pos_clk_te.sum(),)) > 0.01).sum()
    ...
    droplistlen = (np.random.random_sample((all_neg_te.sum(),)) > 0.01).sum()
    drop = np.random.choice(index, droplistlen,replace=False)
    ...
    start=0
    if os.path.isfile(os.path.join(dataset_path, dataset_name) + '/split_train.csv'):
        os.remove(os.path.join(dataset_path, dataset_name) + '/split_train.csv')
    ...
    for epoch_i in range(1):
        ...
        for i, data_ori in enumerate(train_datas):
            ...
            if start==0:
                masked_data.to_csv(os.path.join(dataset_path, dataset_name) + '/split_train.csv',index=False)
            else:
                masked_data.to_csv(os.path.join(dataset_path, dataset_name) + '/split_train.csv',mode='a', header=False,index=False)
