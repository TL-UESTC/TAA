...
def train(tgt_model, optimizer, src_model, src_optimizer, dis_model, dis_optimizer, train_src_loader, train_tgt_loader,
    ...
    for i, data_pack in enumerate(zip(src_loader, tgt_loader)):
        ...
        _, src_feat_tmp = tgt_model(s_categorical_fields, s_numerical_fields)
        if not isinstance(src_feat_tmp, list):
            src_feat_tmp = [src_feat_tmp]*task_num
        
        src_feat = src_feat_tmp

        criticD_real0 = dis_model0(src_feat[0].detach())
        criticD_real0 = criticD_real0.mean()
        criticD_real0.backward(mone)

        criticD_real1 = dis_model1(src_feat[1].detach())
        ...
        criticD_fake0 = dis_model0(tgt_feat[0].detach())
        criticD_fake0 = criticD_fake0.mean()
        criticD_fake0.backward(one)

        gradient_penalty0 = calc_gradient_penalty(dis_model0, src_feat[0].data, tgt_feat[0].data, device)
        gradient_penalty0.backward()

        dis_optimizer0.step()

        #########
        criticD_fake1 = dis_model1(tgt_feat[1].detach())
        criticD_fake1 = criticD_fake1.mean()
        criticD_fake1.backward(one)

        gradient_penalty1 = calc_gradient_penalty(dis_model1, src_feat[1].data, tgt_feat[1].data, device)
        ...
        if n_critic_count >= 5:
            ...
            criticG_fake = dis_model0(tgt_feat[0])
            criticG_fake = criticG_fake.mean()
            G_cost = (-criticG_fake)*learning_e

            criticG_fake1 = dis_model1(tgt_feat[1])
...
def main(tgt_dataset_name,
    ...
    tgt_read_path = src_read_path#f'{read_dir}/{tgt_dataset_name}_{model_name}.pt'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tgt_dataset_name', default='AliExpress_ES',
